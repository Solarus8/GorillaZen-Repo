 	 	...
 	 	if (isset($_POST['submitUpdate']))
 	 	{
			...
			foreach ($_POST['link'] as $link)
			{
				...
				if (isset($_FILES['link_'.$i.'_img']) AND isset($_FILES['link_'.$i.'_img']['tmp_name']) AND !empty($_FILES['link_'.$i.'_img']['tmp_name']))
				{
					...
					if ($error = ImageManager::validateUpload($_FILES['link_'.$i.'_img'], $this->maxImageSize))
						$this->_html .= $error;
				elseif (!$tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS') OR !move_uploaded_file($_FILES['link_'.$i.'_img']['tmp_name'], $tmpName))
					return false;
                elseif (!ImageManager::resize($tmpName, dirname(__FILE__).'/slides/slide_0'.$i.'.jpg'))
	...
	public function _getFormItem($i, $last)
	{
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][url]" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->url:'').'" />
					<p style="clear: both"></p>
				</div>';
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][field1_'.$language['id_lang'].']" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->{'field1_'.$language['id_lang']}:'').'" />
					<p style="clear: both"></p>
				</div>';
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][field2_'.$language['id_lang'].']" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->{'field2_'.$language['id_lang']}:'').'" />
					<p style="clear: both"></p>
				</div>';
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][field3_'.$language['id_lang'].']" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->{'field3_'.$language['id_lang']}:'').'" />
					<p style="clear: both"></p>
				</div>';
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][field4_'.$language['id_lang'].']" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->{'field4_'.$language['id_lang']}:'').'" />
					<p style="clear: both"></p>
				</div>';
		...
		$output .= '
				...
				<div class="margin-form" style="padding-left:0">
					<input type="text" name="link['.$i.'][field5_'.$language['id_lang'].']" size="64" value="'.(isset($this->_xml->link[$i])?$this->_xml->link[$i]->{'field5_'.$language['id_lang']}:'').'" />
					<p style="clear: both"></p>
				</div>';
