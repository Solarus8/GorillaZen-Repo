...
<section class="block blockcms_footer span3">
		...
		<ul class="list-footer toggle_content clearfix">
			<!--{if !$PS_CATALOG_MODE}<li class="first_item"><a href="{$link->getPageLink('return-policy')}" title="{l s='Return Policy' mod='blockcms'}">{l s='Return Policy' mod='blockcms'}</a></li>{/if}
			<li class="{if $PS_CATALOG_MODE}first_{/if}item"><a href="{$link->getPageLink('new-products')}" title="{l s='Guarantee' mod='blockcms'}">{l s='Guarantee' mod='blockcms'}</a></li>
			{if !$PS_CATALOG_MODE}<li class="item"><a href="{$link->getPageLink('best-sales')}" title="{l s='Affiliates' mod='blockcms'}">{l s='Affiliates' mod='blockcms'}</a></li>{/if}
			{if $display_stores_footer}<li class="item"><a href="{$link->getPageLink('stores')}" title="{l s='Privacy' mod='blockcms'}">{l s='Privacy' mod='blockcms'}</a></li>{/if}-->
...
{if $display_poweredby}<section class="bottom_footer">&copy; {$smarty.now|date_format:"%Y"} {l s='Powered by' mod='blockcms'} <a target="_blank" href="http://www.gorillazen.com/index.php?">Gorilla Zen, LLC</a>&trade;</section>{/if}
	<!-- /MODULE Block footer -->
{/if}
