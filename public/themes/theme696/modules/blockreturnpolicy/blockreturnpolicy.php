...
class BlockStore extends Module
{
	public function __construct()
	{
		...
		$this->displayName = $this->l('return policy block');
		$this->description = $this->l('Displays a return policy link directly on your webiste.');
	}
	...
	{
		if (!$this->isCached('blockreturnpolicy.tpl', $this->getCacheId()))
		{
		...
		return $this->display(__FILE__, 'blockreturnpolicy.tpl', $this->getCacheId());
			...
			if (isset($_FILES['store_img']) && isset($_FILES['store_img']['tmp_name']) && !empty($_FILES['store_img']['tmp_name']))
			{
				...
				else
				{
					...
					else
					{
						...
						$this->_clearCache('blockreturnpolicy.tpl');
	...
	public function getContent()
	{
		...
		<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" enctype="multipart/form-data">
			<fieldset>
				<legend>'.$this->l('return policy block configuration').'</legend>';
